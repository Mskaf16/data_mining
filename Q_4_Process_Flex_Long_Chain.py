...
def simulate_performance(n_trials, mu, sigma, sigma2):
    ...
    model.capacity = Param(P, mutable=True)
    ...
    def plant_capacity_rule(model, p):
            return sum(model.x[p,d]*A[p,d] for d in D) <= model.capacity[p]
    model.capacity_upper_bound = Constraint(P, rule=plant_capacity_rule)
    ...
    for i in range(n_trials):
        ...
        capacity_samples = stats.norm.rvs(loc=mu, scale=sigma2, size=6)
        # The negative demands are replaced by 0s
        demand_samples=[max(d,0) for d in demand_samples]
        capacity_samples=[max(p,0) for p in capacity_samples]
    
        # Dedicated case
        sales_per_dedicated_plant = [min(capacity_samples[i], demand_samples[i]) for i in range(len(demand_samples))]
        ...
        # Full flaxible case
        total_sales_full_flex=min(sum(capacity_samples), sum(demand_samples))
        ...
        total_sales_short_chain = \
                min(capacity_samples[0]+capacity_samples[1], demand_samples[0]+demand_samples[1]) + \
                min(capacity_samples[2]+capacity_samples[3], demand_samples[2]+demand_samples[3]) + \
                min(capacity_samples[4]+capacity_samples[5], demand_samples[4]+demand_samples[5])
        ...
        for i in range(len(demand_samples)):
            ...
            model.capacity[P[i]]=capacity_samples[i]
        # Solve the LP model
...
def main():
    ...
    #Capacity: Paraments of normal distributon, mean same as demand
    sigma2=10
    ...
    (sales_dedicated, sales_full_flex, sales_short_chain, sales_long_chain) = simulate_performance(n_trials,mu,sigma,sigma2)
